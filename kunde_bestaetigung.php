...
class Abholung {
	...
	private function getBikeInfo($recnum) {
		...
		$request.=" and `erfassung` ='".$this->erfassung."'";
		$firmaname="";
		...
		while($row=$result->fetch_assoc()) {
			...
			if ($row['status']<=5) {
				...
				$html.='<b>Fahrrad wurde storniert</b>';
			}
			$html.='<input              type="hidden" name="rebikeid"  value="'.$_POST['rebikeid'].'">&nbsp;';
			$html.='<input              type="hidden" name="mail"      value="'.$_POST['mail'].'">&nbsp;';
			$html.='</form>';
		...
		$html2.='<p style="font-size:1.50em;margin:0;"><b>Abhol-Termin:'.$date.'</b></p>';
	...
	function isAllowed(&$db) {		
		return "";
		// Hier alter Wunsch von vorher
		// Nach der änderung keine änderung mehr ermöglichen
		...
		// Abholung bis 3 Werktage vorher noch änderbar
		...
		if (($row['abholtermin_soll'] == NULL) or ($row['abholtermin_soll'] == "0000-00-00 00:00:00")) {
			$dt=new DateTime();
		} else {
			$dt=new DateTime($row['abholtermin_soll']);
		}
		for($i=0;$i<3;$i++) { // 3 Tage vorher nicht mehr änderbar
			while(($dt->modify("-1 days"))->format("N") > 5){}; // 1-7
		}
		
		$dt_now=new DateTime();
		$msg="";
		if ($dt_now > $dt) {
			$msg="Ändern der Daten nur bis 3 Tage vor Abholung möglich.";
		}
		return $msg;
	}			
...
if (!empty($_POST['mail']) and !empty($_POST['rebikeid'])) {
	...
	$request ="SELECT max(erfassung) erfassung from rad_abholung";
	$result=$db->query($request);
	$row=$result->fetch_assoc();
	$erfassung=$row['erfassung'];
	$abh->erfassung=$erfassung;
	
// echo "Erfassung:".$erfassung;	
	...
	$request.=" and `erfassung` ='".$erfassung."'";
...
if (!empty($_POST['info_abholung']) and isset($_POST['info_change'])) {
	...
	if (!$err) {
		$recnumlist=explode(" or recnum=",preg_replace("/^recnum=/i","",$_POST['abholung_recnumlist']));
		
		$log = new Logfile($db);
		$row=array();
		foreach($recnumlist as $v) { 
			$row=array();
			$row['tabelle_recnum']=$v;
			$row['tabelle'] = "rad_abholung";
			$row['feldname'] = "info_abholung";
			$row['wert_neu'] = $_POST['info_abholung'];
			$row['user']     = "(SELECT kundenr from rad_abholung where recnum=".$v.")";  // Hochkommata gehen hier nicht
			$log->add($row);			
		}
...
if ($status>0) {
	...
	if ($msg == "") {
		...
		if ($result=$db->query($request)) {
			...
			$log = new Logfile($db);
			$row=array();
			$row['tabelle_recnum']=$_POST['recnum'];
			$row['tabelle'] = "rad_abholung";
			$row['feldname'] = "status";
			$row['wert_neu'] = $status;
			$row['user']     = "(SELECT kundenr from rad_abholung where recnum='".$_POST['recnum']."')";
			$log->add($row);			
