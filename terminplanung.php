...
if (isset($_POST['anpassen']) or isset($_POST['reset'])) {
	...
	$subrequest ="SELECT recnum from rad_kunde ";
	$subrequest.=" where (`vorname`='".$_POST['vorname']."' and `nachname`='".$_POST['nachname']."' and `plz`='".$_POST['plz']."')";
	$subrequest.=" or `mail`='".$_POST['mail']."'";
	// echo $subrequest;
	// echo "<br>";	
	$result=$db->query($subrequest);
	$kd="";
	while($row=$result->fetch_assoc()) {
		if (!empty($kd)) {
			$kd.=" or ";
		}
		$kd.=" `kundenr`='".$row['recnum']."'";
	}
	

	$request ="update rad_abholung set abholtermin_soll='".$termin."'";
	// $request.=" where recnum='".$_POST['recnum']."'";
	// $request.=" or kundenr = '".$_POST['kundenr']."'";
...
// echo $request;
	$result=$db->query($request);
// Hier noch auswerten wieviele betroffen sind
// matched rows
	
	// $request.=" and radnr>0".$_POST['erfassung']."'";
	
	
	if ($result=$db->query($request)) {
		$msg="Abholungstermin von ".$_POST['vorname']." ".$_POST['nachname']." erfolgreich geändert.<br>";
		list($matched, $changed, $warnings) = sscanf($db->info, "Rows matched: %d Changed: %d Warnings: %d");
		$msg.=$matched." Leasing-Bikes betroffen";
	} else {
...
if (isset($_POST['change']) and !empty($_POST['kundenr'])) {
	$request ="SELECT *,rad_abholung.recnum as recnum from rad_abholung";
	$request.=" LEFT JOIN rad_kunde";
	$request.=" ON rad_abholung.kundenr=rad_kunde.recnum";
	$request.=" WHERE rad_abholung.kundenr ='".$_POST['kundenr']."'" ;
...
// echo $request;
	...
	$html.='<input type="hidden" value="'.$row['plz'].'" name="plz">';
	$html.='<input type="hidden" value="'.$row['mail'].'" name="mail">';
...
$request="SELECT strasse,plz,ort,rad_abholung.recnum as recnum,pos,abholtermin_soll,fahrzeit,km,vorname,nachname,CONCAT(vorname,' ',nachname) as name,mail,count(kundenr) as anz,kundenr,rad_abholung.recnum as abh_recnum from rad_abholung";
$request.=" left join rad_kunde"; 
...
// exit;		
	...
	$html.='<input type="hidden" value="'.$row['kundenr'].'" name="kundenr">';
