...
class Abholung extends Database {
	...
	function getLastErfassung() {
		if (empty($_POST['erfassung'])) {
			$request2 ="SELECT max(erfassung) erfassung from rad_abholung";
			$result=$this->db->query($request2);
			$row=$result->fetch_assoc();
			// $erfassung=$row['erfassung'];
			$_POST['erfassung']=$row['erfassung'];	
			return $row['erfassung'];
		}
		return "";
