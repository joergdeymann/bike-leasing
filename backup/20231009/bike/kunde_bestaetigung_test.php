...
$_POST['mail']="ralf.bahne@vanderlande.com";
$_POST['mail']="D.Rath@sozial-holding.de";
$_POST['rebikeid']="LR-900570";
...
class Abholung {
	...
	private function getBikeInfo($recnum) {
		...
		while($row=$result->fetch_assoc()) {
			...
			$html.='<td>';
			$html.='<form method="POST" action="kunde_bestaetigung_test.php">';
	...
	function isAllowed(&$db) {
		...
		$result=$db->query($request);//		or die('<b>Fehler:</b>'.mysql_error());
if ($result) {
	echo "RESULT OK";
} else {
	// echo mysql_error();
...
echo "isAllowed(): nach Result<br>";

		$row=$result->fetch_assoc();
echo "isAllowed(): vor change_datum<br>";
...
echo "isAllowed(): nach change_datum<br>";
		$dt->modify("+4 hours");
		$dt_now=new DateTime();
		$msg="";
		if ($dt_now > $dt) {
...
if (!empty($msg)) {
	$html="<center>";
	$html.='<h2>'.$msg.'</h2>';
	$html.='<form action="kunde_bestaetigung_test.php" method="POST">';
	$html.='<table>';
...
if (!empty($_POST['info_abholung']) and isset($_POST['info_change'])) {
	$msg=isAllowed($db);
	if ($msg == "") {
		$request="update rad_abholung set info_abholung='".$_POST['info_abholung']."', changed=(changed | 1) where ".$_POST['abholung_recnumlist'];
		// echo $request."<br>";
...
if ($status>0) {
	...
	if ($msg == "") {
		...
		if ($result=$db->query($request)) {
			$msg="Sie haben den Abholstatus in ".array("","Bestätigt","Abgelehnt","Krank","geklaut")[$status]." geäendert";
